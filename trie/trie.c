...
bool check(char *word)
{
    // Make another node and point to root
    node *cursor = root;

    // Itrate through every character of word(user input) and check weather the word matches or not
    ...
    // Return the value of is_word if its true, return true & if it's false return false
